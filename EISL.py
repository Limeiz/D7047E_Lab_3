...
# https://github.com/guangyliu/EISL/blob/main/EISL.py
#
#@article{DBLP:journals/corr/abs-2106-15078,
...
#                  Generation},
#  journal      = {CoRR},
#  volume       = {abs/2106.15078},
#  year         = {2021},
#  url          = {https://arxiv.org/abs/2106.15078},
#  eprinttype    = {arXiv},
#  eprint       = {2106.15078},
#  timestamp    = {Wed, 11 Aug 2021 10:45:08 +0200},
#  biburl       = {https://dblp.org/rec/journals/corr/abs-2106-15078.bib},
#  bibsource    = {dblp computer science bibliography, https://dblp.org}
#}
#
# Removed dependency of Fairseq

class EISLNatCriterion(nn.Module):
    def __init__(self,
#                 task,
...
#            self.weight: Optional[Tensor]
    ...
    def _compute_loss(
            self, outputs, targets, weights, masks=None, label_smoothing=0.0, name="loss", factor=1.0
    ):
        """
        ...
        if masks is not None:
            ...
            weights = 1 if weights == 1 else weights[masks]

        if masks is not None and not masks.any():
            nll_loss = torch.tensor(0)
            loss = nll_loss
        else:
            logits = weights * F.log_softmax(outputs, dim=-1)
            if targets.dim() == 1:
                losses = F.nll_loss(logits, targets.to(logits.device), reduction="none")

            else:  # soft-labels
                losses = F.kl_div(logits,
                                  F.one_hot(targets.to(logits.device), logits.shape[2]).type(dtype=torch.float32),
                                  reduction="none")
                losses = losses.sum(-1)
    ...
    def forward(self, model, sample, reduce=True):
        ...
        for l in losses:
            logging_output[l["name"]] = (
                torch.mean(l["loss"].data / l["factor"])  # utils.item
                if reduce
    ...
    def compute_EISL(self, outputs, targets, masks=None, label_smoothing=0.0, name="loss", factor=1.0):
        ...
        log_probs = weights * F.log_softmax(outputs, dim=-1)
